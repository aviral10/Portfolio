import { Server } from "./interfaces";
export const tempServer: Server = {
    name: "Valhalla",
    image: "https://cdn1.iconfinder.com/data/icons/material-apps/512/icon-android-material-design-512.png",
    channelGroups: [
        {
            name: "General",
            id: 0,
            channelItems: [
                "element 1",
                "element 2",
                "element 3",
                "element 4",
                "element 5",
            ],
        },
        {
            name: "Chamber of Secrets",
            id: 1,
            channelItems: [
                "element 1",
                "element 2",
                "element 3",
                "element 4",
                "element 5",
            ],
        },
    ],
};

export const tempServer_dup: Server = {
    name: "Other Server",
    image: "data:image/png;base64,",
    channelGroups: [
        {
            name: "General1",
            id: 0,
            channelItems: ["element 11", "element 21", "element 31"],
        },
        {
            name: "Chamber of Secrets1",
            id: 1,
            channelItems: ["element 11", "element 21"],
        },
    ],
};
